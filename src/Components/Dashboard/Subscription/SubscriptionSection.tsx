...
import { getSchoolSubscriptionsByUserId, purchaseSubscription, getSubscriptionTransactionsByUserId, downloadInvoiceByTransactionId } from '../../../../services/schoolServices';
...
const subscriptionRes = await getSchoolSubscriptionsByUserId();
const current = pickCurrentSubscription(subscriptionRes);
setSubscription(current);

// Fetch transaction data
const transactionRes = await getSubscriptionTransactionsByUserId();
const transactionData = transactionRes?.data?.transactions || [];
...
try {
const blob = await downloadInvoiceByTransactionId(transactionId);
...
<span className="text-gray-700 font-mono">#{transaction.transaction_id?.slice(-6) || transaction.id}</span>
</div>
...
<span className="font-semibold text-green-600">₹ {Number(transaction.amount).toLocaleString()}</span>
</div>
...
<span className="font-medium">{new Date(transaction.payment_completed_at || transaction.created_at).toLocaleDateString()}</span>
</div>
<div className="col-span-2 md:col-span-1 md:text-right">
<button 
onClick={() => handleDownloadInvoice(transaction.transaction_id)}
className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
>
<Download size={14} />
Download
</button>
